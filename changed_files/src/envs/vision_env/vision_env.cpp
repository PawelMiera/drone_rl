...
bool VisionEnv::reset(Ref<Vector<>> obs) {
  ...
  if(randomise_ball_position)
  {
    for (int i = 0; i < (int)static_objects_.size(); i++)
    {
      // compute relative position vector
      Vector<3> current_pos = base_static_objects_[i]->getPos();
      Vector<3> pos;
      pos << RandomFloat(-3, 3), RandomFloat(-3, 3), RandomFloat(-2, 2);
      Vector<3> out_pos = current_pos + pos;
      bool x_valid = out_pos(QS::POSX) >= world_box_[0] && out_pos(QS::POSX) <= world_box_[1];
      bool y_valid = out_pos(QS::POSY) >= world_box_[2] && out_pos(QS::POSY) <= world_box_[3];
      bool z_valid = out_pos(QS::POSZ) >= world_box_[4] && out_pos(QS::POSZ) <= world_box_[5];
      if (x_valid && y_valid && z_valid)
      {
        static_objects_[i]->setPosition(out_pos);
      }
      else
      {
        static_objects_[i]->setPosition(current_pos);
      }
      
      //std:: cout << static_objects_[i]->getPos() << " ";
    }
  }
...
bool VisionEnv::getObs(Ref<Vector<>> obs) {
  ...
  Scalar delta_pos_x = quad_state_.p(QS::POSX) - init_pos(QS::POSX);
  Scalar delta_pos_y = quad_state_.p(QS::POSY) - init_pos(QS::POSY);
  Scalar warrning_thresh = 2.0;
  Scalar pos_warrning = 0.0;
  Scalar pos_diff_min = quad_state_.p(QS::POSY) - world_box_[2] - warrning_thresh;
  Scalar pos_diff_max = quad_state_.p(QS::POSY) - world_box_[3] + warrning_thresh;

  if(pos_diff_min < 0.0)
  {
    pos_warrning = pos_diff_min / warrning_thresh;
  }
  else if(pos_diff_max > 0.0)
  {
    pos_warrning = pos_diff_max / warrning_thresh;
  }

  // Observations
  //obs << empty_vec, delta_pos_x, quad_state_.p(QS::POSZ), pos_warrning, ori, quad_state_.v, obstacle_obs;     ///quad_state_.p, delta_pos
  obs << delta_pos_x, delta_pos_y, quad_state_.p(QS::POSZ), pos_warrning, ori, quad_state_.v;     ///quad_state_.p, delta_pos
  return true;
}
...
bool VisionEnv::computeReward(Ref<Vector<>> reward) {
  ...
  Scalar touch_collision_penalty = 0.0;
  size_t idx = 0;
  for (size_t sort_idx : sort_indexes(relative_pos_norm_)) {
    if (idx >= visionenv::kNObstacles) break;
    ...
    if (relative_pos_norm_[sort_idx] <=
      ...
      touch_collision_penalty += touch_collision_coeff_;
    }
  ...
  Scalar step_penalty = current_step * step_coeff_;
  Scalar distance_reward = distance_coeff_ * std::pow(1.025, -1.0 * std::abs(60.0 - quad_state_.p(QS::POSX)));

  Scalar too_low_penalty = 0.0;
  float min_height = 0.5;
  if(quad_state_.p(QS::POSZ) < min_height)
  {
    too_low_penalty = too_low_coeff_ * (min_height - quad_state_.p(QS::POSZ));
  }
  Scalar survive = 0.0;
  if(quad_state_.p(QS::POSX) > survive_reward_distance)
  {
...
bool VisionEnv::isTerminalState(Scalar &reward) {
  ...
  if (colision_is_crash && is_collision_) {
     reward = ball_crash_penalty_;;
     return true;
  }

  // simulation time out
  if (cmd_.t >= max_t_ - sim_dt_) {
    reward = timeout_penalty_;
    return true;
  }
  ...
  if (quad_state_.p(QS::POSX) >= end_distance)
  {
    reward = finish_rew_;
    return true;
  }
...
if (cfg["rewards"]) {
    ...
    step_coeff_ = cfg["rewards"]["step_coeff"].as<Scalar>();
    ...
    colision_is_crash = cfg["rewards"]["colision_is_crash"].as<bool>();
    randomise_ball_position = cfg["rewards"]["randomise_ball_position"].as<bool>();

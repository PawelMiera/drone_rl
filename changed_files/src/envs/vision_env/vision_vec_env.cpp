#include "flightlib/envs/vision_env/vision_vec_env.hpp"

namespace flightlib {

template<typename EnvBaseName>
VisionVecEnv<EnvBaseName>::VisionVecEnv() {
  std::string config_path = getenv("FLIGHTMARE_PATH") +
                            std::string("/flightpy/configs/vision/config.yaml");
  cfg_ = YAML::LoadFile(config_path);
  // yaml configurations
  configEnv(cfg_);
}

template<typename EnvBaseName>
VisionVecEnv<EnvBaseName>::VisionVecEnv(const std::string& cfg,
                                        const bool from_file) {
  // load environment configuration
  if (from_file) {
    // load directly from a yaml file
    cfg_ = YAML::LoadFile(cfg);
  } else {
    // load from a string or dictionary
    cfg_ = YAML::Load(cfg);
  }
  configEnv(cfg_);

  change_env_on_reset = cfg_["rewards"]["change_env_on_reset"].as<bool>();
  if(change_env_on_reset)
  {
    this->logger_.info("Loading all env_obstacles");
    std::string difficulty_level_ = cfg_["environment"]["level"].as<std::string>();
    std::string obstacle_cfg_path_= getenv("FLIGHTMARE_PATH") +
                        std::string("/flightpy/configs/vision/") +
                        difficulty_level_ + std::string("/");
    for(int i = 0; i < 101; i++)
    {
      std::string env_name = std::string("environment_") + std::to_string(i);
      std::string current_obstacle_cfg_path_ = obstacle_cfg_path_ + env_name;
      //std::cout << current_obstacle_cfg_path_ << "\n";
      std::vector<std::shared_ptr<UnityObject>> current_dynamic_obs;
      std::vector<std::shared_ptr<UnityObject>> current_static_obs;
      std::string dynamic_object_yaml =
      current_obstacle_cfg_path_ + std::string("/dynamic_obstacles.yaml");
      std::string static_object_csv =
      current_obstacle_cfg_path_ + std::string("/static_obstacles.csv");
      configDynamicObjects(dynamic_object_yaml, current_obstacle_cfg_path_, current_dynamic_obs);
      configStaticObjects(static_object_csv, current_static_obs);
      // for(int k =0; k < 3; k++)
      //    std::cout << current_dynamic_obs[k]->getPos()[0] << " "<< current_dynamic_obs[k]->getPos()[1] << " "<< current_dynamic_obs[k]->getPos()[2] << "\n";
      // std::cout << "\n\nOBJ: \n";
      // for(int k =0; k < 3; k++)
      //   std::cout << current_dynamic_obs[k]->getPos()[0] << " "<< current_dynamic_obs[k]->getPos()[1] << " "<< current_dynamic_obs[k]->getPos()[2] << "\n";
      
      all_envs_dynamic_objects_.push_back(current_dynamic_obs);
      // std::cout << "VEC: \n";
      // for(int j =0; j < all_envs_dynamic_objects_.size(); j++)
      // {
      //   for(int k =0; k < 3; k++)
      //   std::cout <<all_envs_dynamic_objects_[j][k]->getPos()[0] << " "<<all_envs_dynamic_objects_[j][k]->getPos()[1] << " "<<all_envs_dynamic_objects_[j][k]->getPos()[2] << "\n";
      //   std::cout << "\n";
      // }
      all_envs_static_objects_.push_back(current_static_obs);
    }
  }
}

template<typename EnvBaseName>
VisionVecEnv<EnvBaseName>::VisionVecEnv(const YAML::Node& cfg_node) {
  cfg_ = cfg_node;
  configEnv(cfg_);
  random_reset_ = false;
}

template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::reset(Ref<MatrixRowMajor<>> obs) {
  if (obs.rows() != this->num_envs_ || obs.cols() != this->obs_dim_) {
    this->logger_.error(
      "Input matrix dimensions do not match with that of the environment.");
    return false;
  }

  this->receive_id_ = 0;
  for (int i = 0; i < this->num_envs_; i++) {
    this->envs_[i]->reset(obs.row(i), random_reset_);
  }

  return true;
}

template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::reset(Ref<MatrixRowMajor<>> obs, bool random) {
  if (obs.rows() != this->num_envs_ || obs.cols() != this->obs_dim_) {
    this->logger_.error(
      "Input matrix dimensions do not match with that of the environment.");
    return false;
  }
  random_reset_ = random;


  this->receive_id_ = 0;
  for (int i = 0; i < this->num_envs_; i++) {
    this->envs_[i]->reset(obs.row(i), random_reset_);
  }

  return true;
}

template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::step(Ref<MatrixRowMajor<>> act,
                                     Ref<MatrixRowMajor<>> obs,
                                     Ref<MatrixRowMajor<>> reward,
                                     Ref<BoolVector<>> done,
                                     Ref<MatrixRowMajor<>> extra_info) {
  if (act.rows() != this->num_envs_ || act.cols() != this->act_dim_ ||
      obs.rows() != this->num_envs_ || obs.cols() != this->obs_dim_ ||
      reward.rows() != this->num_envs_ || reward.cols() != this->rew_dim_ ||
      done.rows() != this->num_envs_ || done.cols() != 1 ||
      extra_info.rows() != this->num_envs_ ||
      extra_info.cols() != (long int)this->extra_info_names_.size()) {
    this->logger_.error(
      "Input matrix dimensions do not match with that of the environment.");
    std::cout << "act dim  =       [" << act.rows() << "x" << act.cols()
              << "]\n"
              << "obs dim  =       [" << obs.rows() << "x" << obs.cols()
              << "]\n"
              << "rew dim  =       [" << reward.rows() << "x" << reward.cols()
              << "]\n"
              << "done dim  =       [" << done.rows() << "x" << done.cols()
              << "]\n"
              << "extra info dim  =       [" << extra_info.rows() << "x"
              << extra_info.cols() << "]\n"
              << std::endl;
    return false;
  }

#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < this->num_envs_; i++) {
    perAgentStep(i, act, obs, reward, done, extra_info);
  }

  	
  ///////// my code check if all are done:
  //std::cout<<done <<"\n";
  // if(all_done)
  // {
  //   for (int i = 0; i < this->num_envs_; i++) {
  //     this->envs_[i]->reset(obs.row(i), random_reset_);
  //     done[i] = false;
  //   }
  // }
  
  // all_done = true;
  // for (int i = 0; i < this->num_envs_; i++) {
  //   if (!done[i]) {
  //     all_done = false;
  //     break;
  //   }
  // }

  return true;
}

template<typename EnvBaseName>
void VisionVecEnv<EnvBaseName>::perAgentStep(int agent_id,
                                             Ref<MatrixRowMajor<>> act,
                                             Ref<MatrixRowMajor<>> obs,
                                             Ref<MatrixRowMajor<>> reward,
                                             Ref<BoolVector<>> done,
                                             Ref<MatrixRowMajor<>> extra_info) {
  // get individual rewards
  this->envs_[agent_id]->step(act.row(agent_id), obs.row(agent_id),
                              reward.row(agent_id));

  Scalar terminal_reward = 0;
  done[agent_id] = this->envs_[agent_id]->isTerminalState(terminal_reward);

  this->envs_[agent_id]->updateExtraInfo();
  for (int j = 0; j < extra_info.cols(); j++)
    extra_info(agent_id, j) =
      this->envs_[agent_id]->extra_info_[this->extra_info_names_[j]];

  if (done[agent_id]) {

	
    if(change_env_on_reset)
    {
      if(all_envs_dynamic_objects_.size() > 1)
      {
        std::random_device rand_dev;
        std::mt19937 generator(rand_dev());
        std::uniform_int_distribution<int>  distr(0, all_envs_dynamic_objects_.size() - 1);
        int Number = distr(generator);
      
        // int Min = 0;
        // int Max = 100;
        // int Number = std::rand() % (Max + 1 - Min) + Min;
        this->envs_[agent_id]->num_static_objects_ = all_envs_static_objects_[Number].size();
        this->envs_[agent_id]->static_objects_ = all_envs_static_objects_[Number];
        this->envs_[agent_id]->num_dynamic_objects_ = all_envs_dynamic_objects_[Number].size();
        this->envs_[agent_id]->dynamic_objects_ = all_envs_dynamic_objects_[Number];
      }
    }

    this->envs_[agent_id]->reset(obs.row(agent_id), random_reset_);
    reward(agent_id, reward.cols() - 1) = terminal_reward;
  }
}

template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::getQuadAct(Ref<MatrixRowMajor<>> quadact) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
    valid &= this->envs_[i]->getQuadAct(quadact.row(i));
  }
  return valid;
}

template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::getQuadState(Ref<MatrixRowMajor<>> quadstate) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
    valid &= this->envs_[i]->getQuadState(quadstate.row(i));
  }
  return valid;
}


template<typename EnvBaseName>
VisionVecEnv<EnvBaseName>::~VisionVecEnv() {}


///////////////////////////////////////////////////////////
template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::configDynamicObjects(const std::string &yaml_file, std::string obstacle_cfg_path_, std::vector<std::shared_ptr<UnityObject>> &dynamic_objects_) {
  //
  if (!(file_exists(yaml_file))) {
    this->logger_.error("Configuration file %s does not exists.", yaml_file);
    return false;
  }
  YAML::Node cfg_node = YAML::LoadFile(yaml_file);
  // logger_.info("Configuring dynamic objects");
  int num_objects = cfg_node["N"].as<int>();
  // create static objects
  for (int i = 0; i < num_objects; i++) {
    std::string object_id = "Object" + std::to_string(i + 1);
    std::string prefab_id = cfg_node[object_id]["prefab"].as<std::string>();
    std::shared_ptr<UnityObject> obj =
      std::make_shared<UnityObject>(object_id, prefab_id);
    // load location, rotation and size
    std::vector<Scalar> posvec =
      (cfg_node[object_id]["position"]).as<std::vector<Scalar>>();
    std::vector<Scalar> rotvec =
      (cfg_node[object_id]["rotation"]).as<std::vector<Scalar>>();
    std::vector<Scalar> scalevec =
      (cfg_node[object_id]["scale"]).as<std::vector<Scalar>>();
    obj->setPosition(Vector<3>(posvec.data()));
    obj->setRotation(Quaternion(rotvec.data()));
    // actual size in meters
    obj->setSize(Vector<3>(1.0, 1.0, 1.0));
    // scale of the original size
    obj->setScale(Vector<3>(scalevec.data()));
    std::string csv_name = cfg_node[object_id]["csvtraj"].as<std::string>();
    std::string csv_file = obstacle_cfg_path_ + std::string("/csvtrajs/") +
                           csv_name + std::string(".csv");
    if (!(file_exists(csv_file))) {
      this->logger_.error("Configuration file %s does not exists.", csv_file);
      return false;
    }
    obj->loadTrajectory(csv_file);
    dynamic_objects_.push_back(obj);
  }
  // num_dynamic_objects_ = dynamic_objects_.size();
  return true;
}
template<typename EnvBaseName>
bool VisionVecEnv<EnvBaseName>::configStaticObjects(const std::string &csv_file, std::vector<std::shared_ptr<UnityObject>> &static_objects_)
 {
  //
  if (!(file_exists(csv_file))) {
    this->logger_.error("Configuration file %s does not exists.", csv_file);
    return false;
  }
  std::ifstream infile(csv_file);
  int i = 0;
  for (auto &row : CSVRange(infile)) {
    // Read column 0 for time
    std::string object_id = "StaticObject" + std::to_string(i + 1);
    std::string prefab_id = (std::string)row[0];
    //
    std::shared_ptr<UnityObject> obj =
      std::make_shared<UnityObject>(object_id, prefab_id);
    //
    Vector<3> pos;
    pos << std::stod((std::string)row[1]), std::stod((std::string)row[2]),
      std::stod((std::string)row[3]);
    Quaternion quat;
    quat.w() = std::stod((std::string)row[4]);
    quat.x() = std::stod((std::string)row[5]);
    quat.y() = std::stod((std::string)row[6]);
    quat.z() = std::stod((std::string)row[7]);
    Vector<3> scale;
    scale << std::stod((std::string)row[8]), std::stod((std::string)row[9]),
      std::stod((std::string)row[10]);
    //
    obj->setPosition(pos);
    obj->setRotation(quat);
    // actual size in meters
    obj->setSize(Vector<3>(1.0, 1.0, 1.0));
    // scale of the original size
    obj->setScale(scale);
    static_objects_.push_back(obj);
    // base_static_objects_.push_back(obj);
  }
  // num_static_objects_ = static_objects_.size();
  return true;
}
///////////////////////////////////////////////////////////

// IMPORTANT. Otherwise:
// Linker errors because of the separation between the
// declaration and definition of the template class.
// Segmentation fault (core dumped)
template class VisionVecEnv<VisionEnv>;

}  // namespace flightlib